...
#
#
# def box_candidates(box1, box2, wh_thr=1, ar_thr=20, area_thr=0.2):
#     # box1(4,n), box2(4,n)
...
#         self.p = p
#
#     def __call__(self, image, input_dim, mirror_p):
#         image_t = image.copy()
#
#         if mirror_p:
#             image_t = image_t[:, ::-1]
...
class WeakTransform:
    ...
    def __call__(self, image, targets, input_dim, flip):
        targets = np.zeros((self.max_labels, 6), dtype=np.float32)
        if flip:
            image = image[:, ::-1]
        image, r_o = preproc(image, input_dim, self.means, self.std)
