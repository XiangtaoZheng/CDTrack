...
def make_parser():
    ...
    parser.add_argument("--tau", type=int, default=10, help="dti")
...
def write_results_score(filename, results):
    save_format = '{frame},{id},{x1},{y1},{w},{h},1,{c},1\n'
    with open(filename, 'w') as f:
        for i in range(results.shape[0]):
            frame_data = results[i]
            frame_id = int(frame_data[0])
            track_id = int(frame_data[1])
            x1, y1, w, h = frame_data[2:6]
            category = frame_data[7]
            line = save_format.format(frame=frame_id, id=track_id, x1=x1, y1=y1, w=w, h=h, c=category)
            f.write(line)
...
def dti(txt_path, save_path, n_min=25, n_dti=20):
    ...
    for seq_txt in seq_txts:
        ...
        print('min_id: {}, max_id: {}'.format(min_id, max_id))
        print('unique tracks: {}'.format(len(np.unique(seq_data[:, 1]))))
        for track_id in range(min_id, max_id + 1):
            index = (seq_data[:, 1] == track_id)
            tracklet = seq_data[index]
            category = seq_data[0][7]
        ...
        print('left tracks: {}'.format(len(np.unique(seq_results[:, 1]))))
...
if __name__ == '__main__':
    ...
    if args.method == 'linear':
        ...
        GSI(args.txt_path, args.save_path, interval=args.n_dti, tau=args.tau)
