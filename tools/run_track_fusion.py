...
def make_parser():
    ...
    parser.add_argument("--fast-reid-config", dest="fast_reid_config", default=r"",
                        type=str, help="reid config file path")
    parser.add_argument("--fast-reid-weights", dest="fast_reid_weights", default=r"",
                        type=str, help="reid config file path")
...
def oc_sort(predictor, path, args, exp):
    if os.path.isdir(path):
        files = get_image_list(path)
    else:
        files = [path]
...
def main(exp, args):
    ...
    if not args.trt:
        ckpt_file = args.ckpt
        if isinstance(ckpt_file, list):
            logger.info("loading checkpoint")
            ckpt1 = torch.load(ckpt_file[0], map_location="cpu")
            ckpt2 = torch.load(ckpt_file[1], map_location="cpu")
            model.load_state_dict(ckpt1["model"])
            model2.load_state_dict(ckpt2["model"])
            logger.info("loaded {} checkpoint done.".format(ckpt1['start_epoch']))
            logger.info("loaded {} checkpoint done.".format(ckpt2['start_epoch']))
        else:
            logger.info("loading checkpoint")
